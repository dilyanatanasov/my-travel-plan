...
export function legFlightSeconds(km: number): number {
  // NaN slips through Math.min/max clamps (both return NaN), and a NaN
  // duration silently disables the whole SMIL animation — treat it as zero.
  const safeKm = Number.isFinite(km) ? Math.max(km, 0) : 0;
  ...
  return Math.min(5.4, Math.max(1.45, 0.062 * Math.sqrt(safeKm)));
}
